...
def test_the_page_says_not_ready_and_names_the_gaps(client: TestClient, session: Session) -> None:
    version, (subtopic_id,) = _taxonomy(session)
    _question(session, version=version, subtopic_id=subtopic_id, difficulty=Difficulty.EASY)
...
def test_the_page_says_a_gap_cannot_be_requested_directly(
    ...
    """ADR-031 gives the generator the choice, so there can be no fill button."""
    version, (subtopic_id,) = _taxonomy(session)
    _question(session, version=version, subtopic_id=subtopic_id, difficulty=Difficulty.EASY)
...
def test_the_page_states_an_honest_missing_curriculum(client: TestClient) -> None:
    ...
    assert "No curriculum is approved" in body


def test_the_page_freezes_a_set_and_reports_it(client: TestClient, session: Session) -> None:
    version, (subtopic_id,) = _taxonomy(session)
    _question(session, version=version, subtopic_id=subtopic_id)

    response = client.post("/coverage/sets", data={"label": "Autumn 2026"}, follow_redirects=True)

    assert response.status_code == 200
    assert "Question set frozen" in response.text
    assert "Autumn 2026" in response.text


def test_the_page_reports_a_refused_freeze_without_crashing(
    client: TestClient, session: Session
) -> None:
    _taxonomy(session)

    response = client.post("/coverage/sets", data={"label": "Nothing"}, follow_redirects=True)
